...
class PaymentSuccess extends StatefulWidget {
  ...
  dynamic plan;
  PaymentSuccess(
      this.paymentData, this.amount, this.name, this.isWallet, this.plan,
      {super.key});
...
class _PaymentSuccessState extends State<PaymentSuccess> {
  ...
  void initState() {
    ...
    sendStats("payment_success",
        map: {'event': 'payment', "result_for": widget.paymentData});
    if (widget.isWallet) {
  ...
  setPlanStatusToServer(String url) async {
    ...
    map["amount"] = widget.plan['final_cost'];
    map["txn_id"] = widget.paymentData.paymentId.toString();
    map["remark"] = 'android';
    map["plan_id"] = widget.plan['id'];
  ...
  setStatusToServer(String url) async {
    Map map = {};
    map["user_id"] = await getString(key: userId);
    map["amount"] = widget.plan['add_on_percentage'];
    map["txn_id"] = widget.paymentData.paymentId.toString();
    map["remark"] = '';
              ...
              : Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 20),
                      Text(
                        "TXN ID : " + widget.paymentData.paymentId.toString(),
                        style: TextStyle(color: white),
                      ),
                      SizedBox(height: 5),
                      Text(
